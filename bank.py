...
userid=['humaira@gmail.com','sneha@gmail.com','vaishnavi@gmail.com']
userpw=[1234,5678,3490]
userotp=[1230,4560,7890]
emailid=str(input('enter the email id:'))
password=int(input('enter the password:'))
otp=int(input('enter the otp:'))
for i in range(len(userid)):
    if (userid[i]==emailid and userpw[i]==password and userotp[i]==otp):
        print('login successful')
    while (userid[i]==emailid and userpw[i]==password and userotp[i]==otp):# while True
        print("\n1. Deposit")
        print("2. Withdraw")
